...
import { Shield, User as UserIcon, Briefcase, Eye } from 'lucide-react';
...
const UserManagement: React.FC<UserManagementProps> = ({ currentUser }) => {
    ...
    const changeRole = (id: string, newRole: UserRole) => {
        if (currentUser?.role !== 'OpRisk') return; // Only Admin can change roles
        ...
        <div className="space-y-6">
            ...
            {currentUser?.role !== 'OpRisk' && (
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/30 rounded-lg text-sm text-blue-700 dark:text-blue-300">
                    You are viewing this page as <strong>{currentUser?.role}</strong>. Only <strong>OpRisk</strong> admins can modify user roles.
                </div>
            )}
        </div>
    );
